//hellknight chase state
//check health
check_enemy_hp();
//calculate movement
calc_entity_movement();
//attack
var detect_player_dis = global.tile_size * 2;
...
if t1 == SOLID state = red_prince_states.IDLE;
...
if (distance_to_object(o_player) < detect_player_dis) and sign(o_player.x - x) == facing {
	if o_player.hp > 0 {
		if can_attack {
			//attack
			can_attack = false;
			state = red_prince_states.ATTACK;
			image_index = 0;
			image_speed = 1;
		}
		player_alert = true;
...
//	target_y = start_y;
	...
	if abs(x - start_x) < 2 and abs(y - start_y) < 2 state = red_prince_states.IDLE;
}
