...
if (distance_to_object(o_player) < detect_player_dis) and sign(o_player.x - x) == facing and abs(y - o_player.y) < 24 {
	if o_player.hp > 0 {
		if can_attack {
			//attack
			can_attack = false;
			state = fire_golem_states.ATTACK;
			image_index = 0;
			image_speed = 1;
		}
		player_alert = true;
...
//	target_y = start_y;
	...
	if abs(x - start_x) < 2 and abs(y - start_y) < 2 state = fire_golem_states.IDLE;
}

//calculate movement
var _dir = point_direction(x, y, target_x, y);
